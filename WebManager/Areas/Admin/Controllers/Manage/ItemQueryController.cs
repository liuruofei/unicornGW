...
{
    [Authorize]
    public class ItemQueryController : BaseController
    {
        private ICurrencyService _currencyService;
        public ItemQueryController(ICurrencyService currencyService)
        {
        ...
        // GET: /<controller>/
        [UsersRoleAuthFilter("A-111", FunctionEnum.Have)]
        ...
        public IActionResult GetDataSource(string title, int page = 1, int limit = 10)
        {
            ...
            var list = _currencyService.DbAccess().Queryable<YNK_Case>().Where(n => n.Type == 3).WhereIF(!string.IsNullOrEmpty(title), n => n.Title.Contains(title)).ToPageList(page, limit, ref total);
            pageModel.msg = "获取成功";
            pageModel.code = 0;
        ...
        [UsersRoleAuthFilter("A-111", FunctionEnum.Add)]
        ...
        [UsersRoleAuthFilter("A-111", FunctionEnum.Edit)]
        ...
        [UsersRoleAuthFilter("A-111", "Add,Edit")]
        ...
        /// <returns></returns>
        [UsersRoleAuthFilter("A-111", FunctionEnum.Delete)]
