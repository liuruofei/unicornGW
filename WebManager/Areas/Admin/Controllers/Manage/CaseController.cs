...
namespace WebManager.Areas.Admin.Controllers.Manage
{
    ...
    public class CaseController : BaseController
    {
        ...
        public CaseController(ICurrencyService currencyService)
        {
        ...
        public IActionResult GetDataSource(string title, int page = 1, int limit = 10)
        {
            ...
            var list = _currencyService.DbAccess().Queryable<YNK_Case>().Where(n=>n.Type==1).WhereIF(!string.IsNullOrEmpty(title), n => n.Title.Contains(title)).ToPageList(page, limit, ref total);
            pageModel.msg = "获取成功";
            pageModel.code = 0;
